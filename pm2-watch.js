  ...
  static buildMapping() {
    ...
    if (mapMatch) {
      for (let line of mapMatch[1].split("\n")) {
        const m = line.trim().match(/^(\S+)\s+"?(\w+)"?;/);
        if (m) {
          const [_, servicePath, upstream] = m;
          servicePathMap[upstream] = servicePathMap[upstream] || [];
...
const Get_ServicePath = resolver.getServicePathsByPort.bind(resolver);
...
let showEnv = process.argv[3]?.toLowerCase() === "env";
let filterList = null;
...
function renderEnvTable(serviceFolder) {
  return Object.entries(getEnvFileinObj(serviceFolder)).map(
    ([key, value]) => `${chalk.yellow(key)} = ${chalk.green(value)}`
  );
}
...
function scrollOutput(lines, height = 20, header = null) {
  ...
  process.stdin.on("keypress", (_, key) => {
    if (key.name === "down" && offset + height < lines.length) offset++;
    else if (key.name === "up" && offset > 0) offset--;
    else if (key.name === "q" || (key.ctrl && key.name === "c")) safeExit();
    render();
  });
}
  ...
  return new Promise((resolve, reject) => {
    pm2.connect((err) => {
      ...
      pm2.list((err, procs) => {
        ...
        if (!selected.length)
          return reject(new Error(`Process "${filterArg}" not found`));
...
function renderTable(procs) {
  ...
  for (const p of procs) {
    ...
    if (mainPort == port) servicePath = chalk.magentaBright.bold(servicePath);
...
function renderDetails(p) {
  ...
  return [
    ...
    `${chalk.bold("Service Path:")} ${p.pm2_env.status === "online" ? servicePath : "N/A"
    }`,
    `${chalk.bold("Status:")}       ${p.pm2_env.status === "online"
      ? chalk.green.bold("online")
    ...
    `${chalk.bold("Watching:")}     ${p.pm2_env.watch ? chalk.green("enabled") : chalk.gray("disabled")
    }`,
    `${chalk.bold("Instances:")}    ${p.pm2_env.instances || 1}`,
    `${chalk.bold("Max Memory:")}   ${p.pm2_env.max_memory_restart
      ? p.pm2_env.max_memory_restart / 1024 / 1024 + "mb"
      : "N/A"
    }`,
    `${chalk.bold("CPU:")}          ${chalk.blue(p.monit.cpu + "%")}`,
    `${chalk.bold("Memory:")}       ${chalk.green(
...
// Monitor
// =============================
async function startEnvMonitoring() {
  try {
    const selectedProcs = await getPM2List();
    const envLines = [];
    for (const p of selectedProcs) {
      envLines.push(
        ...renderEnvTable(p.pm2_env.pm_cwd),
        selectedProcs.length > 1
          ? `${chalk.red(`End Env File ${p.name}.`)}\n`
          : ""
      );
    }
    scrollOutput(
      envLines,
      30,
      `📦 Environment Variables (${selectedProcs
        .map((p) => chalk.yellow(p.name))
        .join(", ")})`
    );
  } catch (err) {
...
async function startMonitor() {
  try {
    setInterval(async () => {
      ...
      if (filterList && procs.length === 1) {
        output += "\n\n" + renderDetails(procs[0]).join("\n");
      }

      logUpdate(output);
    }, MONITOR_INTERVAL);
  } catch (err) {
    console.error(chalk.red("❌ Failed to start monitor:"), err.message);
    safeExit(2);
  }
}
